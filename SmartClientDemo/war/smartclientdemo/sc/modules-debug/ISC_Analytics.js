...
isc.A.bodyDefaults=isc.addProperties({},isc.ListGrid.getInstanceProperty("bodyDefaults"),{
        selectOnMouseDown:function(record,rowNum,colNum){
            ...
            if(!(this.selection.cellIsSelected(rowNum,colNum)&&isc.EH.rightButtonDown())){
                ...
                if(rv.showRowFacetLabels!=false&&rv.rowFacetLabels)rv.rowFacetLabels.selection.deselectAll();
...
isc.B.push(isc.A.initWidget=function isc_CubeGrid_initWidget(){
    ...
    if(this.initialCriteria&&this.dataSource){
        ...
        if(this.data!=null&&!(isc.isAn.Array(this.data)&&this.data.length==0)){
...
,isc.A.setEditValues=function isc_CubeGrid_setEditValues(cellID,editValues,suppressDisplay,a,b,c,d){
    var rowNum=cellID[0],
        colNum=cellID[1];
    ...
    return this.invokeSuper(isc.CubeGrid,"setEditValues",cellID,editValues,suppressDisplay,
                            a,b,c,d);
}
,isc.A.setEditValue=function isc_CubeGrid_setEditValue(rowNum,colNum,value,suppressDisplay,suppressChange){
    ...
    return this.Super("setEditValue",[rowNum,colNum,value]);
}
...
,isc.A.findRowNum=function isc_CubeGrid_findRowNum(record,editSession){
    if(this.cellIndex==null)this.indexCellRecords();
...
,isc.A.setRows=function isc_CubeGrid_setRows(rows,facetsUnchanged,dontRemapEdits){
    ...
    if(this.isDrawn()){
        ...
        this.layoutChildren();
        this.map("addChild",this.rowHeaders);
        this._markBodyForRedraw("setRows");
...
,isc.A.setColumns=function isc_CubeGrid_setColumns(columns,facetsUnchanged,dontRemapEdits){
    ...
    if(this.isDrawn()){
        ...
        if(!facetsUnchanged)this.createColumnFacetLabels();
        this.createColumnHeaderBars(true);
        this.setFields(this.fields,this.getFieldWidths());
        this.layoutChildren();
        this.map("addChild",this.colHeaders);
        this._markBodyForRedraw("setColumns");
        this.syncHeaderScrolling(null,null,true);
    }
    this.cellIndex=null;
    if(!dontRemapEdits&&this._editSessions&&!isc.isA.emptyObject(this._editSessions)){
...
,isc.A.scrollToFacetValues=function isc_CubeGrid_scrollToFacetValues(facetValues,center){
    ...
    this.scrollCellIntoView(row!=-1?row:null,
                            column!=-1?column:null,center,true);
}
...
,isc.A.indexCellRecords=function isc_CubeGrid_indexCellRecords(){
    ...
    if(!this.batchedIndexing||this.data.length<batchSize){
        ...
        var batches=Math.floor(this.data.length/batchSize);
        for(var i=0;i<batches;i++){
            var lastBatch=(i==batches-1),
                startIndex=i*batchSize;
            this._makeIndexingTimer(startIndex,batchSize,lastBatch);
        }
    }
...
,isc.A._makeIndexingTimer=function isc_CubeGrid__makeIndexingTimer(startIndex,batchSize,lastBatch){
    var cg=this;
        isc.Timer.setTimeout(function(){
            cg.addRecordsToIndex(cg.data,startIndex,
                                 lastBatch?cg.data.length:startIndex+batchSize);
            if(lastBatch){
                cg._indexingInProgress=null;
                cg.doneIndexing();
...
,isc.A.addRecordsToIndex=function isc_CubeGrid_addRecordsToIndex(data,startIndex,endIndex){
    ...
    for(var i=startIndex;i<endIndex;i++){
        ...
        if(this.inlinedFacet&&row>=0&&col>=0){
            this.addInlinedValues(cellRecord,row,col);
        }
    }
    if(this.logIsDebugEnabled("cgTiming")){
...
,isc.A.getFacetValuesOffset=function isc_CubeGrid_getFacetValuesOffset(facetValues,facetIds,outerFields,isCellRecord,sparseValues){
    ...
    var isRow;
    if(!sparseValues){
        isRow=(facetIds.last()==this.rowFacets.last());
    }else{
        isRow=this.rowFacets.contains(facetIds.last());
    }
    if(isRow!=this.groupByRow)return field.coordinate;
...
isc.evalBoundary;isc.B.push(isc.A.getBorderStyles=function isc_CubeGrid_getBorderStyles(fields,rowBorders){
    var borders=[];
    for(var i=0;i<fields.length;i++){
        var field=fields[i];
        if(field==null)continue;
...
,isc.A.bodyDrawing=function isc_CubeGrid_bodyDrawing(){
    ...
    if(this.dataSource&&this.autoFetchData)this._cgFetchData();
}
...
,isc.A.fetchData=function isc_CubeGrid_fetchData(criteria,callback,requestProperties){
    ...
    this.setData([]);
    this._cgFetchData(null,null,callback,requestProperties);
}
...
,isc.A.layoutChildren=function isc_CubeGrid_layoutChildren(){
    ...
    if(this.haveColumnFacets()){
        ...
        if(rowFacetLabels&&rowFacetLabels.isDrawn()){
            innerHeaderHeight=rowFacetLabels.getVisibleHeight();
        }
    }
        ...
        for(var i=0;i<this.columnFacets.length;i++){
            var facet=this.getFacet(this.columnFacets[i]),
                ...
                facetHeight=(isInnerMost?innerHeaderHeight:
                                  (facet.height!=null?facet.height:this.facetHeight));
...
isc.evalBoundary;isc.B.push(isc.A.createColumnHeaderBars=function isc_CubeGrid_createColumnHeaderBars(dontAddChild){
    ...
    for(var i=0;i<this.colFields.length;i++){
        ...
        if(headerBar==null){
            ...
            var headerBar={
                ...
                height:(facet.height!=null?facet.height:this.facetHeight),
...
isc.evalBoundary;isc.B.push(isc.A.getSelectedCells=function isc_CubeGrid_getSelectedCells(){
        ...
        items.addList([
            {title:"Rename...",
             click:"target.setFacetValueTitle('"+facetId+"','"+facetValues[facetId]+
                  "',prompt('Enter the new name for this facet value:','"+this.getFacetValue(facetId,facetValues[facetId]).title+"'))"
            }
        ]);
    }
...
isc.evalBoundary;isc.B.push(isc.A.moveFacet=function isc_CubeGrid_moveFacet(facetId,intoRows,index){
    var location=this.getFacetLocation(facetId);
    if(location==null){
