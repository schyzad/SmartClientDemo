...
var transformRules = {
    ...
    iconsMap: {
        ...
        "LeftFillTriangleIcon": "/actions/back.png",
        "RightFillTriangleIcon": "/actions/forward.png",
    ...
    widgetPropertyTranslations: {
        ...
        "com.balsamiq.mockups::TitleWindow": {
            ...
            afterInit : function (name, control) {
                ...
                if (control.headerControls) {
                    if (control.showMinimizeButton) control.headerControls.add("minimizeButton"); 
                    if (control.showMaximizeButton) control.headerControls.add("maximizeButton"); 
                    if (control.showCloseButton) control.headerControls.add("closeButton");                    
                }
            },
            ...
            addControl : function (container, headerControl) {
                ...
                if (controlAreaName == "header") {
                    if (container.headerControls == null) {
                        container.headerControls = ["headerIcon","headerLabel"];
                    }
                    container.headerControls.add("ref:" + control.ref);
                } else {
        ...
        "com.balsamiq.mockups::Accordion": {
            ...
            afterInit : function (name, control) {
                ...
                delete control.sections.widgets;
            },
            ...
            controlPropertiesParser : function (name, value) {
                if (name == "text") {
                    ...
                    for (var i = 0; i < res.length; i++) {
                        var key = isc.MockDataSource.convertTitleToName(res[i], transformRules.BMMLImporter.fieldNamingConvention);
                        if (r[key]) {
                            if (indexes[key]) {
                                indexes[key]++;
                            } else {
        ...
        "com.balsamiq.mockups::DataGrid": {
            ...
            afterInit : function (name, control) {
                ...
                var row1 = value.split("\n")[0];
                var vs = row1.split(",");
                var maxRows = 1;
                for(var i = 0; i < vs.length; i++) {
                    if (vs[i].endsWith(" ^") || vs[i].endsWith(" v") ||
                            vs[i].endsWith(" ^v")) {
                        var newVsi = vs[i].substring(0, vs[i].length - 2);
                        control.dataSource.MockDataSource.mockData = 
                            control.dataSource.MockDataSource.mockData.replace(vs[i], newVsi);
                    }
                ...
                var fields = isc.MockDataSource.parseTableFields(value, transformRules.BMMLImporter.fieldNamingConvention);
                var fieldsCorrect = isc.MockDataSource.parseTableFields(control.dataSource.MockDataSource.mockData, transformRules.BMMLImporter.fieldNamingConvention);
                ...
                var cacheData = isc.MockDataSource.parseTable(value);
        ...
        "com.balsamiq.mockups::CheckBoxGroup": {
            ...
            estimateControlSize : function (control) {
                ...
                for (var i = 0; i < control.items.length; i++) {
                    if (control.items[i].title) {
                        largestWidth = 
                            Math.max(largestWidth, 8 * control.items[i].title.length);                        
                    } else {
                        largestWidth = 
                            Math.max(largestWidth, 8 * control.items[i].value.length);
                    }
                }
                return {
        ...
        "com.balsamiq.mockups::BarChart": {
            afterInit : function (name, control) {
                ...
                // hide labels for data as it is in balsamiq
        ...
        "com.balsamiq.mockups::Calendar": {
            afterInit : function (name, control) {
                // Calendar widget min.size is 166x152
                if (control.height < 166) {
                    control.height = 166;                    
                }
                if (control.width < 152) {
                    control.width = 152;                    
                }
            }
        }
